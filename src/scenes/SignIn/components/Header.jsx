import styled from 'styled-components';
import { route } from 'preact-router';
import openLoginPopup from 'feathers-authentication-popups';
import { Field, reduxForm } from 'redux-form';
import { Flex, Box } from 'reflexbox';
import get from 'lodash/get';
import LinkedIn from 'react-linkedin-login/src/';
import FacebookLogin from 'react-facebook-login/dist/facebook-login-render-props';

/**
 |--------------------------------------------------
 | HeaderWrapper
 |--------------------------------------------------
 */

const HeaderWrapper = styled.div`
  position: relative;
  background-color: #5C6BC0;
  min-height: 200px;
`;

/**
 |--------------------------------------------------
 | CloseButton
 |--------------------------------------------------
 */

const CloseButton = styled.i`
  position: absolute;
  color: #fff;
  padding: 15px;
  font-size: 28px!important;
  top: 0;
  left: 0;
  cursor: pointer;
`;

/**
 |--------------------------------------------------
 | Title
 |--------------------------------------------------
 */

const Title = styled.h1`
  text-align: center;
  color: white;
  font-size: 22px;
  padding: 20px 60px;
  margin: 0;
  font-weight: 500;
`;

/**
 |--------------------------------------------------
 | Inputs
 |--------------------------------------------------
 */

const Input = styled.input`
  background-color: white;
  border-radius: 2px;
  border: none;
  padding: 10px;
  margin-bottom: 10px;
  width: 100%;
  outline: none;
`;

/**
 |--------------------------------------------------
 | Block
 |--------------------------------------------------
 */

const Block = styled.div`
  padding: 0 20px;
`;

/**
 |--------------------------------------------------
 | LoginButon
 |--------------------------------------------------
 */

const LoginButon = styled.button`
  background: none;
  border: 1px solid #fff;
  border-radius: 2px;
  padding: 10px;
  width: 100%;
  color: #fff;
  outline: none;

  &:active {
    background-color: white;
    color: #5C6BC0;
  }
`;

/**
 |--------------------------------------------------
 | FacebookLoginButton
 |--------------------------------------------------
 */

const FacebookLoginButton = styled.button`
  border-radius: 2px;
  padding: 10px;
  width: 100%;
  color: #fff;
  outline: none;

  background: #3b5998;
  border: none;
  margin-top: 10px;
  box-shadow: 0px 0px 4px 0px #0000003b;
  display: flex;
  justify-content: space-around;
`;

/**
 |--------------------------------------------------
 | LinkedinLoginButton
 |--------------------------------------------------
 */

const LinkedinLoginButton = styled.button`
  border-radius: 2px;
  padding: 10px;
  width: 100%;
  color: #fff;
  outline: none;

  background: #3b5998;
  border: none;
  margin-top: 10px;
  box-shadow: 0px 0px 4px 0px #0000003b;
  display: flex;
  justify-content: space-around;
`;

/**
 |--------------------------------------------------
 | ForgotPasswordLabel
 |--------------------------------------------------
 */

const ForgotPasswordLabel = styled.p`
  color: #fff;
  text-align: center;
  font-size: 13px;
  padding: 20px 0;
  margin: 0;
`;

const callbackLinkedIn = ({ code, redirectUri }) => ({
  // Login with linkedin
});

/**
 |--------------------------------------------------
 | Header
 |--------------------------------------------------
 */

const InputWrapper = ({ input, ...props }) => <Input {...input} {...props} />;

let Form = () => (
  <form>
    <Field name="email" component={InputWrapper} placeholder="E-mail" type="text" />
    <Field name="password" component={InputWrapper} placeholder="Senha" type="password" />
  </form>
);


Form = reduxForm({ form: 'signin' })(Form);

const Header = ({ authenticate, formData }) => (
  <HeaderWrapper>
    <CloseButton className="material-icons" onClick={() => route('/home')}>close</CloseButton>
    <Block>
      <Title>Entre na sua conta para continuar</Title>

      <div style={{ maxWidth: 500, margin: 'auto' }}>
        <Form />
        <LoginButon onClick={() => {
          window.tempEmail = get(formData, 'email', '').toLowerCase();
          window.tempPassword = get(formData, 'password', '');
          formData.email = formData.email || '';
          formData.email = formData.email.toLowerCase();
          authenticate(Object.assign({ strategy: 'local' }, formData), {
            password: { required: { message: 'A senha é obrigatória!' } },
            email: { required: { message: 'O campo email é obrigatório!' }, email: { message: 'Informe um email válido!' } }
          });
        }}
        >
                    Entrar
        </LoginButon>


        <Choose>
          <When condition={window.cordova}>
            <FacebookLoginButton onClick={() => {
              if (window.cordova) {
                facebookConnectPlugin.login(['public_profile', 'email'],
                  (result) => {
                    console.log(result);
                    authenticate({
                      strategy: 'facebook-token',
                      access_token: result.authResponse.accessToken
                    });
                    console.log('entrou no ', result.authResponse.accessToken);
                  }, console.log);
              }
              else {
                window.location.replace('https://mysterious-oasis-88871.herokuapp.com/auth/facebook');
              }
            }}
            >
              <Flex align="center">
                <img width="20" height="20" style={{ marginRight: 10 }} src="data:image/svg+xml;utf8;base64," />
                                Login pelo Facebook
              </Flex>
            </FacebookLoginButton>
          </When>
          <Otherwise>
            <FacebookLogin
              style={{
                width: '100%',
                display: 'grid',
                border: '1px solid #fff'
              }}
              appId="293374761096403"
              autoLoad={false}
              fields="name,email,picture"
              textButton="login com facebook"
              onClick={result => {
                if (result.accessToken) {
                  authenticate({
                    strategy: 'facebook-token',
                    access_token: result.accessToken
                  });
                }
              }}
              callback={result => {
                authenticate({
                  strategy: 'facebook-token',
                  access_token: result.accessToken
                });
              }}
              isMobile
              disableMobileRedirect
              render={renderProps => (
                <FacebookLoginButton onClick={renderProps.onClick}>
                  <Flex align="center">
                    <img width="20" height="20" style={{ marginRight: 10 }} src="data:image/svg+xml;utf8;base64," />
                                        Login pelo Facebook
                  </Flex>
                </FacebookLoginButton>
              )}
            />
          </Otherwise>
        </Choose>

        {/*<LinkedinLoginButton onClick={() => {*/}
          {/*const onError = function (e) { console.error('LinkedIn Error: ', e); };*/}
          {/*const onSuccess = function (r) {*/}
            {/*console.log('LinkedIn Response: ', JSON.stringify(r));*/}

            {/*// check for existing session*/}
            {/*cordova.plugins.LinkedIn.getActiveSession(function(session) {*/}
              {/*if (session) {*/}
                {/*console.log('LinkedIn Response SESSION: ', JSON.stringify(session));*/}
                {/*authenticate({*/}
                  {/*strategy: 'linkedin-token',*/}
                  {/*access_token: session.accessToken*/}
                {/*});*/}
                {/*console.log('We have an active session');*/}
                {/*console.log('Access token is: ', session.accessToken);*/}
                {/*console.log('Expires on: ', session.expiresOn);*/}
              {/*} else {*/}
                {/*console.log('There is no active session, we need to call the login method');*/}
              {/*}*/}
            {/*});*/}
          {/*};*/}

          {/*cordova.plugins.LinkedIn.login(['r_basicprofile', 'r_emailaddress'], true, (result) => {*/}
            {/*// get connections*/}
            {/*cordova.plugins.LinkedIn.getRequest('people/~', onSuccess, onError);*/}

            {/*result*/}

            {/*// share something on profile*/}
            {/*// see more info at https://developer.linkedin.com/docs/share-on-linkedin*/}
            {/*const payload = {*/}
              {/*comment: 'Hello world!',*/}
              {/*visibility: {*/}
                {/*code: 'anyone'*/}
              {/*}*/}
            {/*};*/}
            {/*// cordova.plugins.LinkedIn.postRequest('~/shares', payload, onSuccess, onError);*/}
          {/*}, onError);*/}
        {/*}}*/}
        {/*>*/}
              {/*Login pelo linkedin*/}
        {/*</LinkedinLoginButton>*/}


        {/*<LinkedIn*/}
          {/*clientId="77em4yiqqzq1z0"*/}
          {/*callback={callbackLinkedIn}*/}
          {/*redirectUri="http://localhost:8080/#/signin"*/}
          {/*className={LinkedinLoginButton}*/}
          {/*text="LinkedIn"*/}
        {/*/>*/}

        <ForgotPasswordLabel onClick={() => route('/forgotpassword')}>
                    Esqueceu sua Senha?
        </ForgotPasswordLabel>
      </div>

    </Block>
  </HeaderWrapper>
);

export default Header;
