  ...
  <Box style={{ backgroundColor: "#DEDEEF", padding: "16px 45px" }}>
    <h2>
      {window.PENDING_NEW_REQUEST
        ? "Dúvida postada com sucesso"
        : "Que pena, você não está cadastrado!"}
    </h2>
    <h3>
      {window.PENDING_NEW_REQUEST ? (
        <span>
          Seu problema está a um passo de ser visualizado pelos advogados, você
          só precisa se{" "}
          <Link href="/signup" onClick={onSignUpBySearchClick}>
            cadastrar
          </Link>{" "}
          ou fazer{" "}
          <Link href="/signin" onClick={onSignUpBySearchClick}>
            login
          </Link>{" "}
          no sistema!
        </span>
        ...
        <span>
          ...
          <Link href="/signintype" onClick={onSignUpBySearchClick}>
            cadastre
          </Link>{" "}
          ou faça{" "}
          <Link href="/signin" onClick={onSignUpBySearchClick}>
...
class Search extends Component {
  ...
  //SORT USERS BY PREMIUM, ARTICLES, VIDEOS AND OTHERS
  sortPersonListItemByRules = ResultData => {
    return _.orderBy(
      ResultData,
      [
        "subscription.amount",
  ...
  render({ toggleDrawer, search, result, chatsUnread, isUserSignedIn }) {
    return (
            ...
            <When condition={isUserSignedIn}>
              ...
              <Scroll>
                <If condition={result && result.data.length && !this.loading}>
                  <PersonList>
                    {this.sortPersonListItemByRules(result.data)
                      .filter(l => l.user)
                      .map(lawyer => (
                        <PersonListItem
                          key={lawyer._id}
                          ...
                          oab={lawyer.oab}
                          distance={distance2(lawyer.dist.calculated)}
                        />
                      ))}
                  </PersonList>
                </If>
                ...
                >
                  <EmptyList
                    text="nenhum advogado encontrado"
                    icon="help"
                    pb={4}
                    pf={4}
                  />
